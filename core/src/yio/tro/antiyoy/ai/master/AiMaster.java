...
//        gameController.speedManager.setSpeed(0);
    ...
    void updateActionThirst(MasterAction masterAction) {
        switch (masterAction.maType) {
            default:
                break;
            case cut_tree:
                masterAction.thirst = 0.5 + 2 * currentProvince.countObjects(Obj.PALM) + currentProvince.countObjects(Obj.PINE) + 3 * currentProvince.countObjects(Obj.REVOLT);
                break;
            case peacefully_expand:
    ...
    Unit mergeUnits(Unit kamikaze, Unit target) {
        if (kamikaze.strength>=5 || target.strength >= 5) {
            return null;
        }
    ...
    boolean isSpendingValid(PsType psType) {
        switch (psType) {
            ...
            case unit5:
                return money >= 2 * GameRules.PRICE_UNIT;
            case unit6:
                return money >= 12 * GameRules.PRICE_UNIT;
            case unit7:
                return money >= 5 * GameRules.PRICE_UNIT;
            case unit8:
                return money >= 3 * GameRules.PRICE_UNIT;
            case unit9:
                return money >= 4 * GameRules.PRICE_UNIT;
