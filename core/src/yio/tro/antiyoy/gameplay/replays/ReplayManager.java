...
public class ReplayManager {
    ...
    public void onRevoltSpawned(Hex hex) {
        if (!canAddAction()) return;
        replay.addAction(new RaRevoltSpawned(hex));
    }
    ...
    public void onMountainBuilt(Hex hex) {
        if (!canAddAction()) return;
        replay.addAction(new RaMountainBulit(hex));
    }

    public void onFortBuilt(Hex hex) {
        if (!canAddAction()) return;
        replay.addAction(new RaFortBuilt(hex));
    }

    public void onCityBuilt(Hex hex) {
        if (!canAddAction()) return;
        replay.addAction(new RaCityBuilt(hex));
    }

    public void onHillBuilt(Hex hex) {
        if (!canAddAction()) return;
        replay.addAction(new RaHillBuilt(hex));
    }
    ...
    public void onStopButtonPressed() {
        gameController.matchStatistics.turnsMade=0;
