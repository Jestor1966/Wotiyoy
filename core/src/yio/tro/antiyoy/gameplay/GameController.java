...
public class GameController {
    ...
    private void checkToEndTurn() {
        ...
        //System.out.println("Turns:"+getMatchStatistics().turnsMade);
        turnStartActions();
    }
    ...
    public static int maxNumberFromArraySiege(int array[]) {
        int[] arr = new int[GameRules.MAX_FRACTIONS_QUANTITY];
        int[] arrTemp = new int[GameRules.MAX_FRACTIONS_QUANTITY];
        int maxFraction=0;
        ...
        for(int i=0;i<arrTemp.length;i++){
            if(arr[arrTemp.length-1]==arrTemp[i]){
                return i;
            }
        }
        for(int i:arr){
    ...
    private void turnStartActions() {
        //fieldManager.tryFortToControl();
        if(GameRules.eventEnabled){
            if(!(GameRules.aiOnlyMode) || !(GameRules.replayMode)){
                eventManager.initRepeats();
            }
        }
        ...
        if (isCurrentTurn(0)) {
            ...
            fieldManager.archerKill();
            fieldManager.crossbowKill();
            fieldManager.gunboatKill();
            fieldManager.expandTrees();
        }

        prepareCertainUnitsToMove();
        // this must be called before 'check for bankrupts' and after 'expand trees'
        fieldManager.transformGraves();
    ...
    public void checkToApplyAdditionalData() {
        ...
        eventManager.checkToApplyAdditionalData();
    ...
    private void moveUnitWithAttack(Unit unit, Hex destination, Province unitProvince) {
        ...
        if(destination.containsSiege()){
            return;
        }else{
            if (!destination.canBeAttackedBy(unit) || unitProvince == null) {
                System.out.println("Problem in GameController.moveUnitWithAttack");
                //System.out.println(unit.strength+"|"+destination.objectInside);
                Yio.printStackTrace();
                return;
            }
            fieldManager.setHexFraction(destination, turn);
            fieldManager.cleanOutHex(destination);// must be called before object in hex destroyed
    ...
    public int strengthTransfer(int temp){
        if(temp>=5){
            if(temp==5) {
                temp=1;
            }else if(temp==6) {
    ...
    public int moneyTransfer(int temp){
        if(temp>=5){
            if(temp==5) {
                temp=1;
            }else if(temp==6) {
