...
public abstract class Ruleset {
    ...
    public abstract boolean canSpawnRevoltOnHex(Hex hex);
    ...
    public abstract void onRevoltWin(Province province);
    ...
    public int howManyRevoltNearby(Hex hex) {
        if (!hex.active) return 0;
        int c = 0;
        for (int i = 0; i < 6; i++)
            if (!(hex.getAdjacentHex(i).containRevoltCantDestory())) c++;
        return c;
    }
}
