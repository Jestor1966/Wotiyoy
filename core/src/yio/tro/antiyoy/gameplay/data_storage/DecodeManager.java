...
public class DecodeManager {
    ...
    private void applyEditorInfo() {
        ...
        if (split.length > 3) {
            GameRules.eventEnabled = Boolean.valueOf(split[3]);
        }
        if (split.length > 4) {
            GameRules.enableRebel = Boolean.valueOf(split[4]);
        }
        if (split.length > 5) {
            GameRules.captainAsCityRules = Boolean.valueOf(split[5]);
        }
    }
    ...
    public boolean extractEventEnable(String levelCode) {
        source = levelCode;
        String section = getSection("editor_info");
        if (section == null) return false;

        String[] split = section.split(" ");
        return Boolean.valueOf(split[3]);
    }
