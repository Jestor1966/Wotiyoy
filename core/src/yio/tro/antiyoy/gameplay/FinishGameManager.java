...
public class FinishGameManager implements EncodeableYio{
    ...
    public void perform() {
        ...
        switch (goalType) {
            ...
            case reach_target_income:
                ...
                break;
            case most_hexes_in_turn:
                applyMostHexesInTurn();
                break;
        }
    }
    ...
    private boolean areDefaultConditionsForced() {
        ...
        if (goalType == GoalType.most_hexes_in_turn) return false;
    ...
    private void applyMostHexesInTurn() {
        ...
        if (gameController.matchStatistics.turnsMade < arg1 - 1) return;
        int[] winnerHexesNum = new int[GameRules.MAX_FRACTIONS_QUANTITY];
        int[] fractionsModel = new int[GameRules.MAX_FRACTIONS_QUANTITY];
        for(int i=0;i<=GameRules.MAX_FRACTIONS_QUANTITY-1;i++){
            fractionsModel[i]=i;
        }
        for(Province province:gameController.fieldManager.provinces){
            for(Hex hex:province.hexList){
                winnerHexesNum[hex.fraction]=winnerHexesNum[hex.fraction]+1;
            }
        }
        for(int i=0;i<winnerHexesNum.length-1;i++) {
            for(int j=0;j<winnerHexesNum.length-i-1;j++) {
                if(winnerHexesNum[j]>winnerHexesNum[j+1]) {
                    int temp=winnerHexesNum[j];
                    winnerHexesNum[j]=winnerHexesNum[j+1];
                    winnerHexesNum[j+1]=temp;

                    int temp2=fractionsModel[j];
                    fractionsModel[j]=fractionsModel[j+1];
                    fractionsModel[j+1]=temp2;
                }
            }
        }

        System.out.println(winnerHexesNum[winnerHexesNum.length-1]);
        winnerFraction=fractionsModel[fractionsModel.length-1];
        Province biggestProvince = fieldManager.getBiggestProvince(winnerFraction);
        if (biggestProvince == null) {
            System.out.println("Problem:FinishGameManager.applyMostHexesInTurn");
            return;
        }
        endGame(winnerFraction);
    }
