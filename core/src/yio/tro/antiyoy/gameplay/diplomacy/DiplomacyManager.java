...
public class DiplomacyManager {
            ...
            if (objectInside > 0 && objectInside != Obj.TOWN) {
                ...
                switch (objectInside) {
                    ...
                    case Obj.TOWER:
                        ...
                        break;
                    case Obj.CITY:
                        replayManager.onCityBuilt(hex);
                        break;
                    case Obj.FORT:
                        replayManager.onFortBuilt(hex);
                        break;
                }
            }
        }
    ...
    public boolean isWar(Hex hex1, Hex hex2) {
        if(!GameRules.diplomacyEnabled) return true;
        DiplomaticEntity attacker = getEntity(hex1.fraction);
        DiplomaticEntity defender = getEntity(hex2.fraction);
    ...
    public void makeAllies(DiplomaticEntity initiator, DiplomaticEntity entity, int duration) {
        if (!initiator.alive || !entity.alive) return;
        if (initiator.getRelation(entity) == DiplomaticRelation.FRIEND) return;
        if (GameRules.diplomaticRelationsLocked) return;

        // should be before relations change because they will influence dotations
        DiplomaticContract contract = addContract(DiplomaticContract.TYPE_ALLY, initiator, entity);
        contract.setExpireCountDown(duration);
        removeContract(DiplomaticContract.TYPE_PIECE, initiator, entity);

        initiator.setRelation(entity, DiplomaticRelation.ALLY);
        entity.setRelation(initiator, DiplomaticRelation.ALLY);
