...
public class LevelEditorManager {
    ...
    private void inputModeDeletePlusActions(Hex focusedHex) {
        if (!focusedHex.active) return;
        Hex temp;
        deactivateHex(focusedHex);
        for(int i=0;i<6;i++){
    ...
    private void inputModeSetObjectActions(Hex focusedHex) {
        ...
        else if (inputObject == 8) { // mountain
            addSolidObject(focusedHex, lastObject);
            checkToTurnIntoFarm(focusedHex);
            gameController.addAnimHex(focusedHex);
        }
        else if (inputObject == 9) { // hill
            addSolidObject(focusedHex, lastObject);
            checkToTurnIntoFarm(focusedHex);
            gameController.addAnimHex(focusedHex);
        }
        else if (inputObject == 10) { // fort
            addSolidObject(focusedHex, lastObject);
            checkToTurnIntoFarm(focusedHex);
            gameController.addAnimHex(focusedHex);
        }
        else if (inputObject == 11) { // city
            addSolidObject(focusedHex, lastObject);
            checkToTurnIntoFarm(focusedHex);
            gameController.addAnimHex(focusedHex);
        }
        else if (inputObject == 12) { // revolt
            addSolidObject(focusedHex, lastObject);
            checkToTurnIntoFarm(focusedHex);
            gameController.addAnimHex(focusedHex);
        }
    ...
    private void addSolidObject(Hex focusedHex, int lastObject) {
        ...
        if (lastObject == Obj.MOUNTAIN) {
            gameController.addSolidObject(focusedHex, inputObject);
            return;
        }
        if (lastObject == Obj.FORT) {
            gameController.addSolidObject(focusedHex, Obj.FORT);
            return;
        }
        if (lastObject == Obj.CITY) {
            gameController.addSolidObject(focusedHex, Obj.CITY);
            return;
        }
    ...
    private void tryToAddUnitToFocusedHex(Hex focusedHex, int unitStrength) {
        ...
        //System.out.println(inputObject+" "+unitStrength+" "+defStr+" "+str);
    ...
    public boolean onTouchDown(int x, int y) {
        ...
        updateFocusedHex();
        if (leInputMode == LeInputMode.set_object || leInputMode == LeInputMode.set_hex || leInputMode == LeInputMode.delete|| leInputMode == LeInputMode.delete_plus) {
    ...
    public boolean onTouchDrag(int x, int y) {
        ...
        if (leInputMode == LeInputMode.set_hex || leInputMode == LeInputMode.delete || leInputMode == LeInputMode.delete_plus) {
            focusedHexActions(gameController.fieldManager.focusedHex);
        }
    ...
    public void onEditorEventButtonPressed() {
        gameController.yioGdxGame.menuControllerYio.hideAllEditorPanels();
        Scenes.sceneEditorEvent.create();
    ...
    public void onLevelImported(String levelCode) {
        ...
        gameController.eventManager.onLevelImported(levelCode);
