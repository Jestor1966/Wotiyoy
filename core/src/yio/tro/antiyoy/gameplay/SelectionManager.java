...
public class SelectionManager {
    ...
    public void applyUnitSelection(Unit unit) {
        int moveFix=0,temp=0;
        selectedUnit = unit;
        temp=selectedUnit.strength;
        MoveZoneManager moveZoneManager = gameController.fieldManager.moveZoneManager;
    ...
    private boolean isHexGoodForDefenseTip(Hex hex) {
        ...
        if (hex.objectInside == Obj.CITY) return true;
        if (hex.objectInside == Obj.FORT) return true;
    ...
    private boolean reactionBuildStuff() {
        ...
        if (canBuildOnHex(focusedHex, tipType)) {
            ...
            if (unitBuildConditions()) {
                ...
                fieldManager.buildUnit(fieldManager.selectedProvince, focusedHex, temp);
    ...
    public int getCurrentTipPrice() {
        switch (tipType) {
            ...
            case SelectionTipType.UNIT_5:
                return (GameRules.PRICE_UNIT * 2);
            case SelectionTipType.UNIT_6:
                return (GameRules.PRICE_UNIT * 12);
            case SelectionTipType.UNIT_7:
                return (GameRules.PRICE_UNIT * 5);
            case SelectionTipType.UNIT_8:
                return (GameRules.PRICE_UNIT * 3);
            case SelectionTipType.UNIT_9:
                return (GameRules.PRICE_UNIT * 4);
