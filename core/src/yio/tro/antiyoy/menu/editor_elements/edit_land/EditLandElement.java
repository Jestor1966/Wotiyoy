...
public class EditLandElement extends InterfaceElement{
    ...
    private void updateItemDeltas() {
        ...
        for (int i = 0; i < 6; i++) {
            alignLeft(getItem(i), i, 5);
            alignLeft(getItem(i + 6), i, 4);
            alignLeft(getItem(i + 12), i, 3);
            alignLeft(getItem(i + 18), i, 2);
            alignLeft(getItem(i + 24), i, 1);
            alignLeft(getItem(i + 30), i, 0);
        }


        alignRight(getItem(EleActionType.delete), 0, 5);
    ...
    private void alignLeft(EleItem eleItem, int x, int y) {
        ...
        float stepY = (float) (position.height / 8);
    ...
    private void applyTargetItem() {
        ...
        switch (targetItem.actionType) {
            ...
            case def:
                ...
                break;
            case delete_plus:
                levelEditorManager.setInputMode(LeInputMode.delete_plus);
                break;
            case set_hex_plus:
                if (levelEditorManager.setHexPlus==true){
                    levelEditorManager.setHexPlus=false;
                }else{
