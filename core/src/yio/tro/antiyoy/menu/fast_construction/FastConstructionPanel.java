...
public class FastConstructionPanel extends InterfaceElement {
    ...
    public RectangleYio position,position2, viewPosition;
    ...
    private void initItems() {
        ...
        addItem(FcpActionType.unit_5);
        addItem(FcpActionType.unit_6);
        addItem(FcpActionType.unit_8);
    ...
    private void initMetrics() {
        height = 0.07f * GraphicsYio.height;
    }
    ...
    private void initPosition() {
        ...
        position2.x = GraphicsYio.width * (3/4);
        position2.y = 0;
        position2.width = GraphicsYio.width /4;
        position2.height = height * 2;
    }
    ...
    private void rearrangeSpecialItems() {
        ...
        endTurnItem.delta.y = 3.2f * height;
        ...
        diplomacyItem.delta.y = 4.5f * height;
    ...
    private void rearrangeByNormalRules() {
        placeItem(FcpActionType.strong_tower, 0, 1,false);
        placeItem(FcpActionType.tower, 1, 1,false);
        placeItem(FcpActionType.farm, 2, 1,false);

        placeItem(FcpActionType.unit_1, 3, 1,true);
        placeItem(FcpActionType.unit_2, 2, 1,true);
        placeItem(FcpActionType.unit_3, 1, 1,true);
        placeItem(FcpActionType.unit_4, 0, 1,true);
        placeItem(FcpActionType.unit_5, 2, 2,true);
        placeItem(FcpActionType.unit_6, 1, 2,true);
        placeItem(FcpActionType.unit_8, 0, 2,true);
    }


    private void rearrangeBySlayRules() {
        placeItem(FcpActionType.tower, 0, 2,false);

        placeItem(FcpActionType.unit_1, 3, 2,true);
        placeItem(FcpActionType.unit_2, 2, 2,true);
        placeItem(FcpActionType.unit_3, 1, 2,true);
        placeItem(FcpActionType.unit_4, 0, 2,true);
    }


    private void placeItem(FcpActionType actionType, int placeX, int placeY,boolean alignRight) {
        FcpItem itemByAction = getItemByAction(actionType);
        if (itemByAction == null) return;

        itemByAction.visible = true;
        if (alignRight) {
            itemByAction.delta.x = (float) (position.width - (1.4 * itemTouchOffset + placeX * (2.5f * itemTouchOffset)));
        } else {
            itemByAction.delta.x = 1.4f * itemTouchOffset + placeX * (2.5f * itemTouchOffset);
        }
        itemByAction.delta.y = (height / 2) * placeY + (placeY-1) * 0.4f * height;
    }
    ...
    private void applyBuildUnit(int strength) {
        ...
        switch (strength) {
            ...
            case 4:
                ...
                break;
            case 5:
                tipType = SelectionTipType.UNIT_5;
                break;
            case 6:
                tipType = SelectionTipType.UNIT_6;
                break;
            case 7:
                tipType = SelectionTipType.UNIT_7;
                break;
            case 8:
                tipType = SelectionTipType.UNIT_8;
                break;
            case 9:
                tipType = SelectionTipType.UNIT_9;
                break;
        }
